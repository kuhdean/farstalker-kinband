...
const operativeSelectionGrid = document.getElementById(
  "operative-selection-grid",
);
const equipmentSelectionGrid = document.getElementById(
...
const factionRulesAccordionContainer = document.getElementById(
  "faction-rules-accordion-container",
);
const strategicPloysAccordionContainer = document.getElementById(
  "strategic-ploys-accordion-container",
);
const firefightPloysAccordionContainer = document.getElementById(
  "firefight-ploys-accordion-container",
);
...
const chosenEquipmentCardContainer = document.getElementById(
  "chosen-equipment-card-container",
);
...
function loadState() {
  ...
  if (rosterData) {
    try {
      activeRoster = JSON.parse(rosterData);
    } catch {
      activeRoster = [];
    }
  }
  if (gameData) {
    try {
      ...
      gameState = { turn: 1, cp: 2, vp: 0, totalEp: 10, spentEp: 0 };
    }
  }
  if (equipData) {
    try {
      killTeamEquipment = JSON.parse(equipData);
    } catch {
      killTeamEquipment = [];
    }
  }
...
function addKillTeamEquipment(name) {
  const item = [...factionEquipment, ...universalEquipment].find(
    (eq) => eq.name === name,
  );
  if (!item) return;
...
function validateRosterAddition(opData) {
  if (activeRoster.length >= 12)
    return { isValid: false, message: "Roster is full (12 operatives max)." };
  if (opData.isLeader && activeRoster.some((op) => op.isLeader))
    return { isValid: false, message: "Roster can only have one Leader." };
  if (opData.isUnique && activeRoster.some((op) => op.id === opData.id))
    return {
      isValid: false,
      message: `${opData.name} is a unique operative and is already in the roster.`,
    };
  if (
    opData.id === "hound" &&
    activeRoster.filter((op) => op.id === "hound").length >= 2
  )
    return {
      isValid: false,
      message: "You can only include a maximum of two Kroot Hounds.",
    };
...
function validateFullRoster() {
  ...
  const eqValid =
    gameState.spentEp <= gameState.totalEp &&
    killTeamEquipment.length <= MAX_EQUIPMENT_ITEMS;
...
function renderReferenceAccordions() {
  ...
  factionRulesAccordionContainer.innerHTML = makeSection(
    "Faction Rules",
    factionRules,
  );
  strategicPloysAccordionContainer.innerHTML = makeSection(
    "Strategic Ploys",
    strategicPloys,
    true,
  );
  firefightPloysAccordionContainer.innerHTML = makeSection(
    "Firefight Ploys",
    firefightPloys,
    true,
  );
...
function parseKeywords(rulesString) {
  ...
  Object.keys(keywordGlossary).forEach((keyword) => {
    ...
    const regex = new RegExp(
      `\\b${baseKeyword.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`,
      "gi",
    );
    processedString = processedString.replace(
      regex,
      `<span class="keyword">${baseKeyword}</span>`,
    );
  });
...
function renderAllOperativeCards() {
  ...
  activeRoster.forEach((op) => {
    ...
    const modifiedMoveDisplay = isInjured
      ? `${Math.max(1, modifiedMoveVal - 2)}"`
      : op.stats.move;
    const modifiedAPLDisplay = isInjured
      ? Math.max(1, op.stats.apl - 1)
      : op.stats.apl;
    // Save usually doesn't change with injury. BS/WS handled per weapon.

    let tagsHTML = "";
    if (op.isLeader)
      tagsHTML += `<span class="operative-tag leader-tag">LEADER</span>`;
    if (op.isUnique)
      tagsHTML += `<span class="operative-tag unique-tag">UNIQUE</span>`;

    const weaponsHTML = op.weapons
      .map((w) => {
        let skillDisplay = w.skill;
        if (w.skill.endsWith("+")) {
          // Check if it's a roll target
          const baseSkill = parseInt(w.skill);
          if (!isNaN(baseSkill)) {
            skillDisplay = isInjured
              ? `<span class="stat-modifier-inline">${Math.min(6, baseSkill + 1)}+</span>`
              : w.skill;
          }
        }
    ...
    const abilitiesHTML = op.abilities
      .map(
        (a) => `<li><strong>${a.name}:</strong> ${parseKeywords(a.text)}</li>`,
      )
      .join("");
    const actionsHTML = (op.uniqueActions || [])
      .map(
        (a) => `<li><strong>${a.name}:</strong> ${parseKeywords(a.text)}</li>`,
      )
      .join("");
    ...
    killTeamEquipment.forEach((eq) => {
      let eligible = true;
      if (eq.ineligible_ids && eq.ineligible_ids.includes(op.id))
        eligible = false;
    ...
    const contextButtonsHTML = contextActions
      .map((n) => `<button class="context-btn">${n}</button>`)
      .join("");
...
function changeWounds(instanceId, amount) {
  ...
  operative.currentWounds = Math.max(
    0,
    Math.min(operative.stats.wounds, operative.currentWounds + amount),
  );
  const isNowInjured = operative.currentWounds <= operative.stats.wounds / 2;

  if (
    wasInjured !== isNowInjured ||
    (amount !== 0 &&
      (operative.currentWounds === 0 ||
        operative.currentWounds === operative.stats.wounds))
  ) {
    renderAllOperativeCards();
  } else {
...
function updateCardVisualState(instanceId) {
  ...
  const healthPercentage = Math.max(
    0,
    (operative.currentWounds / operative.stats.wounds) * 100,
  );
  ...
  card.classList.add(
    operative.order === "Conceal" ? "order-conceal" : "order-engage",
  );

  if (operative.isActivated && operative.currentWounds > 0) {
    // Don't add if incapacitated
...
function showTooltip(keyword, event) {
  ...
  const foundKey = Object.keys(keywordGlossary).find((k) =>
    keyword.toLowerCase().includes(k.split(" ")[0].toLowerCase()),
  );
...
function bindStaticEventListeners() {
  ...
  document.body.addEventListener("click", (e) => {
    ...
    if (target.matches(".cp-mod"))
      modifyStateValue("cp", parseInt(target.dataset.amount));
    else if (target.matches(".vp-mod"))
      modifyStateValue("vp", parseInt(target.dataset.amount));
    else if (target.matches(".order-toggle")) toggleOrder(target.dataset.id);
    else if (target.matches(".activation-status"))
      toggleActivation(target.dataset.id);
    else if (target.closest(".health-bar")) {
      const bar = target.closest(".health-bar");
      handleHealthClick(bar.dataset.id, e, bar);
    } else if (target.matches(".remove-eq-btn"))
      removeKillTeamEquipment(target.dataset.name);
    else if (target.matches(".ploy-button"))
